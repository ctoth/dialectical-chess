...
# ==========================================================================
# The cut seam — the generic graph builder names no chess objection kind.
# ==========================================================================
...
@pytest.mark.unit
def test_generic_graph_builder_names_no_chess_objection_kind() -> None:
    """The seam — ``opinion_graph`` (the generic graph builder) references no
    chess-specific objection-kind name. The chess kinds (``QUEEN_BLUNDER``,
    ``MOVED_PIECE_EN_PRIS``, ``FLANK_PAWN_*``, ...) are cartridge; the generic
    builder reads only generic typed evidence."""
    names = _names_used(_PACKAGE / "opinion_graph.py")
    chess_kind_names = {kind.name for kind in ObjectionKind}
    leaked = names & chess_kind_names
    assert leaked == set(), f"chess objection kinds leaked into opinion_graph: {leaked}"


@pytest.mark.unit
def test_generic_decider_names_no_chess_objection_kind() -> None:
    """The seam — ``decide`` (the generic lexicographic decider) references no
    chess-specific objection-kind name; the material-safety FACT term is
    supplied by the chess ``suppression`` policy."""
    names = _names_used(_PACKAGE / "decide.py")
    chess_kind_names = {kind.name for kind in ObjectionKind}
    leaked = names & chess_kind_names
    assert leaked == set(), f"chess objection kinds leaked into decide: {leaked}"


@pytest.mark.unit
def test_generic_graph_builder_does_not_import_suppression_internals() -> None:
    """The seam — ``opinion_graph`` depends on the chess suppression policy
    through exactly one hook, ``suppressing_defeaters``; it does not reach
    into chess-kind helpers."""
    tree = ast.parse((_PACKAGE / "opinion_graph.py").read_text(encoding="utf-8"))
    suppression_imports: set[str] = set()
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom) and node.module == "dialectical_chess.suppression":
            suppression_imports = {alias.name for alias in node.names}
    assert suppression_imports == {"suppressing_defeaters"}
